...
void rtosInit()
{
  ...
  // REQUIRED: systick for 1ms system time;
  NVIC_ST_CTRL_R=0;
 NVIC_ST_CTRL_R=0X07;
...
void yield()
{
	...
	__asm("           PUSH  {R4-R11, LR}");
		tcb[taskCurrent].sp=  (void*)load(tcb[taskCurrent].sp);
		   taskCurrent = rtosScheduler();
		  tcb[taskCurrent].sp= (void*)restore(tcb[taskCurrent].sp);
	    __asm("     ADD SP, #4");
	    __asm("     POP   {R4-R11, PC}");
...
token(char str[80])
{
	...
	//splitting the string
			...
			{
				putsUart0("The PID is ");
			int conv = atoi((uint32_t)tcb[taskCurrent].pid);
			 char buffer;
				buffer = tostring(conv);
				putsUart0(buffer);
				putsUart0("\r\n");
